...
scaler = torch.tensor([28.3, 62.52179487, 25.0]).to(dev)
...
with torch.no_grad():
    ...
    for xb, yb in test_dl:
        batch_test_loss = loss_func(
            best_model(xb.to(dev)) * scaler, yb.to(dev) * scaler, reduction="none"
        )
        batch_test_loss_2 = loss_func_2(
            best_model(xb.to(dev)) * scaler, yb.to(dev) * scaler, reduction="none"
        )
