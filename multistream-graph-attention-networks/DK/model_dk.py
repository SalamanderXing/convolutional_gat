...
class GAT_Multi_Head(nn.Module):
    def __init__(self, nfeat, nhid, alpha, nheads):
        ...
        self.attentions = [
            GraphAttentionLayer(in_features=nfeat, out_features=nhid, alpha=alpha)
...
class GraphAttentionLayer2D(nn.Module):
    ...
    def forward(self, h):
        ...
        h_prime = torch.matmul(h_prime.double(), adj_mat_norm_d12).view(
...
class GAT_Multi_Head2D(nn.Module):
    def __init__(self, nfeat, nhid, alpha, nheads):
        ...
        self.attentions = [
            GraphAttentionLayer2D(in_features=nfeat, out_features=nhid, alpha=alpha)
            for _ in range(nheads)
        ]
        for i, attention in enumerate(self.attentions):
            self.add_module("attention_{}".format(i), attention)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.data_bn = nn.BatchNorm1d(4 * 5)
        self.l1 = GAT_Multi_Head(120, 6, 0.2, 20)
