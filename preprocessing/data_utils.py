...
# ----------------------------------
def get_triple_idxs_w_blacklist(
    days,
    bins_to_predict=32,
    day_bins=96,
    len_seq_in=4,
    ...
    for d in days:
        for i in range(in_range):
            ...
            if d in black_list.keys():
                ...
                timebins_in_black = [
                    idx for idx in seq_bins if idx in black_list[d]
                ]
...
def get_time():
    return [
        "{}{}{}{}00".format("0" * bool(i < 10), i, "0" * bool(j < 10), j)
...
def get_next_day(sorted_days, current_day):
    ...
    assert pos_date + 1 < len(
        sorted_days
    ), f" Error: date {current_day} is the last date, there is not a next day"
...
def read_and_normalize_h5(file_path, target_vars):
...
def postprocess(data, fill_value, max_value, add_offset, scale_factor):
    ...
    assert (
        data.max() <= max_value
    ), f"Error, postprocess of the variables is wrong"
...
def preprocess_fn(data, fill_value, max_value, add_offset, scale_factor):
    ...
    assert (
        0 <= np.nanmin(data) and np.nanmax(data) <= 1
    ), f"Error, the scale of the variables is wrong"
...
def read_netcdf4(product, day_in_year, time, root=""):
    if root == "":
        print(
            "reading from shared file systen, you might want to inform the root variable to boost I/O"
        )
    ...
    assert (
        len(file_path) == 1
    ), f" Error with file in {path} ----> all these files were found: {file_path}"
...
def get_products_netcdf4(
    ...
    if ct_1hot is not None:
        ...
        attrs_order = [
            ct_1hot[key] for key in sorted(ct_1hot.keys(), reverse=False)
        ]
    else:
...
def get_sequence_netcdf4(
    ...
    for time_bin in range(in_start_id, in_start_id + len_seq):
        if (
            time_bin >= day_bins
        ):  # this is to load next days' timebin in the test split
            time_bin = time_bin % day_bins
            if not already_next_day:
                print(
                    f"Since input sequence goes from {in_start_id} to {in_start_id+len_seq}"
                )
                print(f"we should need to update day {day_id}...")
                next_day_id = get_next_day(sorted_dates, day_id)
                print(
                    f"to {next_day_id}, but files are all in the folder of the former day.\n"
                )
                # already_next_day = True

        prod, masks, channels = get_products_netcdf4(
            day_id,
            hhmmss[time_bin],
            products,
            path,
