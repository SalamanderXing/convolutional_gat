...
class Encoder(nn.Module):
    def __init__(self, in_channels=4, use_inception=True, repeat_per_module=1):
        super().__init__()
        # stages
...
class Decoder(nn.Module):
    def __init__(self, out_channels=6, use_inception=True, repeat_per_module=1):
        ...
        self.downch3 = self._create_upsampling_module(64, 3)
    ...
    def forward(self, x):
        ...
        # print(f"{out.shape=}")
        out = self.downch3(self.stage3(out))[:, :, :80, :80]
        # print(f"{out.shape=}")
        out = self.stage4(out)
        # print(f"{out.shape=}")
        out = t.sigmoid(self.last(out))
        # print(f"{out.shape=}")
        return out
