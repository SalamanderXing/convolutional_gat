...
class GATLayerTemporal(nn.Module):
    def __init__(self, image_size, in_features, out_features, alpha, concat=True):
        ...
        self.a = nn.Parameter(
            torch.zeros(size=(2 * self.img_height * self.img_width, 1))
        )
    ...
    def forward(self, input, adj):
        ...
        a_input = torch.cat(
            [
                Wh.repeat(1, 1, nodes, 1).view(
                    nodes * nodes, height, width, self.out_features
                ),
                Wh.repeat(nodes, 1, 1, 1),
            ],
            dim=1,
        ).view(nodes, nodes, self.out_features, 2 * height * width)
