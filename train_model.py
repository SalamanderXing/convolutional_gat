...
ap.add_argument(
    ...
    "--dataset_path",
    required=True,
    help="path to the .pkl dataset, e.g. 'data/dataset.pkl'",
)
...
ap.add_argument(
    ...
    type=int,
    required=True,
    help="number of timesteps to be included in the input",
)
ap.add_argument(
    "-pt",
    "--predict_timestep",
    type=int,
    required=True,
    help="number of timesteps (hours) ahead for the prediction",
)
...
scaler_min = torch.tensor([0.000e00]).to(dev)
scaler_max = torch.tensor([240.0]).to(dev)

model = Model()
model = model.double().to(dev)

train_dl, valid_dl = get_train_valid_loader(
    dataset_path, input_timesteps, predict_timestep
)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if mean_valid_loss < best_mean_valid_loss:
            torch.save(
                model.state_dict(),
                "trained_models/best_model_nl_" + date_time_string + ".pt",
            )
            print(
...
best_model.load_state_dict(
    torch.load("trained_models/best_model_nl_" + date_time_string + ".pt")
)
