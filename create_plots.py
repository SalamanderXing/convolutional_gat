            ...
            for k in range(len(x)):
                ...
                if raininess >= 0.5:
                    ...
                    for model_obj, model_file in models:
                        model = model_obj
                        model.load_state_dict(t.load(os.getcwd() + '/convolutional_gat/experiments/' + model_file + '/model.pt'))
                        # model = t.jit.load()
                        model.to(device)
                        model.eval()
                        preds = model(x)
                    ...
                    plt.savefig(f"{os.getcwd()}/convolutional_gat/models_comparison/pred_{k}.png")
                    plt.close()
                    # model.train()
